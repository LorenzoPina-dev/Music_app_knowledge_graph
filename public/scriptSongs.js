document.addEventListener("DOMContentLoaded", function () {
    ...
    const nomePlaylist = url.searchParams.get('nomePlaylist');
    const useSpotify = url.searchParams.get('useSpotify');
    document.getElementById("nomePlaylist").innerHTML = nomePlaylist;
    let urlApi=`http://localhost:3000/api/getSongPlaylist?idPlaylist=${idPlaylist}`;
    if(useSpotify==true) 
...
function renderData(data){
    ...
    data.forEach(e => {
        ...
        if(e.album==undefined) {
            album=encodeURIComponent(e.album_name);
        } else 
            album=e.album.name;
