...
document.addEventListener("DOMContentLoaded", function () {
    const url = new URL(window.location.href),
          codiceArtista = url.searchParams.get('idAutore'),
          nomeCanzone = url.searchParams.get('nomeSong'),
          nomeAutore = url.searchParams.get('nomeAutore');
    ...
    const nc = FormatSongName(nomeCanzone),
          na = FormatArtistName(nomeAutore);
...
function renderData(data) {
    console.log(data)
    const d = data.map(d => d.nomeCanzone.value);
    console.log(d);
    ...
    const divGeneri = document.getElementById('divgeneri');
